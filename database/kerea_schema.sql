-- ============================================================
-- KEREA Corporate Website — Complete MySQL Database Schema
-- Version: 1.0.0
...
-- DATABASE
-- ────────────────────────────────────────────────────────────
CREATE DATABASE IF NOT EXISTS `kerea_db`
...
-- ────────────────────────────────────────────────────────────
-- 5. SITE SETTINGS (replaces settings.json)
-- ────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS `news_articles` (
    ...
    `excerpt`      TEXT            NULL,
    `content`      LONGTEXT        NULL,
    `category`     VARCHAR(100)    NOT NULL DEFAULT 'general' COMMENT 'Industry News|Press Release|Policy Update|Success Story',
...
CREATE TABLE IF NOT EXISTS `publications` (
    ...
    `category`     VARCHAR(100)    NOT NULL DEFAULT 'report' COMMENT 'report|standard|brief|guide',
    `year`         YEAR            NULL,
...
-- ────────────────────────────────────────────────────────────
-- 13. DOWNLOADS (general downloadable resources)
-- ────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS `events` (
    ...
    `description`  TEXT            NULL,
    `content`      LONGTEXT        NULL,
    `event_type`   VARCHAR(100)    NOT NULL DEFAULT 'conference' COMMENT 'conference|workshop|summit|expo|training|webinar',
...
CREATE TABLE IF NOT EXISTS `training_programmes` (
    ...
    `description`  TEXT            NULL,
    `content`      LONGTEXT        NULL,
    `duration`     VARCHAR(200)    NULL COMMENT 'e.g. 3 Days, 6 Weeks',
    `delivery_mode`VARCHAR(100)    NULL COMMENT 'in-person|online|hybrid',
    `level`        VARCHAR(100)    NULL COMMENT 'beginner|intermediate|advanced',
    `fee`          DECIMAL(10,2)   NULL DEFAULT 0.00,
...
CREATE TABLE IF NOT EXISTS `partners` (
    ...
    `partner_type`VARCHAR(100)    NOT NULL DEFAULT 'strategic' COMMENT 'strategic|implementing|donor|technical',
...
CREATE TABLE IF NOT EXISTS `team_members` (
    ...
    `department`  VARCHAR(200)    NULL COMMENT 'Board|Secretariat|Technical',
    `sort_order`  SMALLINT        NOT NULL DEFAULT 0,
    `is_active`   TINYINT(1)      NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `success_stories` (
    ...
    `impact`      VARCHAR(500)    NULL COMMENT 'short impact statement',
    `status`      ENUM('published','draft') NOT NULL DEFAULT 'draft',
    `featured`    TINYINT(1)      NOT NULL DEFAULT 0,
    `created_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `menus` (
    ...
    `location`   VARCHAR(100) NOT NULL UNIQUE COMMENT 'header|footer_col1|footer_col2',
    `created_at` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `menu_items` (
    ...
    `parent_id`   INT UNSIGNED NULL COMMENT 'self-ref for dropdowns',
    `label`       VARCHAR(300) NOT NULL,
    `url`         VARCHAR(1000)NOT NULL,
    `target`      VARCHAR(20)  NOT NULL DEFAULT '_self' COMMENT '_self|_blank',
...
-- ────────────────────────────────────────────────────────────
-- 23. CONTACT MESSAGES (form submissions)
-- ────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS `activity_log` (
    ...
    `entity_type` VARCHAR(100)    NULL COMMENT 'news|event|user|setting…',
...
-- Super Admin user (password: Admin@KEREA2026)
INSERT INTO `users` (`role_id`, `first_name`, `last_name`, `email`, `password_hash`, `status`, `email_verified`) VALUES
(1, 'KEREA', 'Administrator', 'admin@kerea.org',
 '$2y$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uHV/SiU.', -- password: Admin@KEREA2026
 'active', 1);

-- Note: The hash above is a placeholder. Run install/index.php to create a proper hash.

-- Site Settings (all keys)
INSERT INTO `site_settings` (`setting_key`, `setting_val`, `setting_type`, `label`, `group_name`, `sort_order`) VALUES
...
('social_facebook',  '#',                          'text',    'Facebook URL',          'social',  1),
('social_twitter',   '#',                          'text',    'Twitter/X URL',         'social',  2),
('social_linkedin',  '#',                          'text',    'LinkedIn URL',          'social',  3),
...
INSERT INTO `hero_slides` (`title`, `subtitle`, `cta_text`, `cta_url`, `bg_color`, `sort_order`, `is_active`) VALUES
...
INSERT INTO `news_articles` (`author_id`, `title`, `slug`, `excerpt`, `category`, `status`, `visibility`, `featured`, `published_at`) VALUES
...
-- Sample Events
INSERT INTO `events` (`title`, `slug`, `description`, `event_type`, `venue`, `location`, `start_date`, `end_date`, `status`, `visibility`, `featured`) VALUES
('Solar Tech Expo 2026',
 'solar-tech-expo-2026',
 'East Africa\'s premier renewable energy exhibition bringing together industry leaders, innovators, and policymakers.',
...
-- Sample Partners
INSERT INTO `partners` (`name`, `description`, `website_url`, `partner_type`, `sort_order`, `is_active`, `featured`) VALUES
...
('GIZ Kenya', 'Deutsche Gesellschaft für Internationale Zusammenarbeit', 'https://giz.de', 'donor', 4, 1, 1);

-- Sample FAQs
INSERT INTO `faqs` (`question`, `answer`, `category`, `sort_order`, `is_active`) VALUES
('What is KEREA?',
 'KEREA (Kenya Renewable Energy Association) is the primary representative body for all sustainable energy practitioners and corporate stakeholders across East Africa. We champion industry standards, policy advocacy, and member empowerment.',
 'general', 1, 1),
...
('What are the membership tiers?',
 'KEREA offers Corporate, Associate, and Individual membership tiers, each with specific benefits tailored to your organisation\'s needs and size.',
 'membership', 3, 1),
...
-- Sample Testimonials
INSERT INTO `testimonials` (`name`, `title`, `organisation`, `quote`, `rating`, `sort_order`, `is_active`, `featured`) VALUES
('Dr. Amina Hassan', 'CEO', 'SolarLink Technologies',
 'KEREA\'s certification process gave us the credibility we needed to win major government contracts. Their standards are world-class.',
 5, 1, 1, 1),
...
-- Menus (default navigation structure)
INSERT INTO `menus` (`name`, `location`) VALUES
('Header Navigation', 'header'),
('Footer — KEREA Connect', 'footer_connect'),
('Footer — Sector Governance', 'footer_governance');

-- Header Menu Items
INSERT INTO `menu_items` (`menu_id`, `label`, `url`, `target`, `sort_order`, `is_active`) VALUES
(1, 'Home', '/', '_self', 1, 1),
...
-- Footer Governance Menu Items
INSERT INTO `menu_items` (`menu_id`, `label`, `url`, `target`, `sort_order`, `is_active`) VALUES
(3, 'Policy & Advocacy', '/policy-advocacy/', '_self', 1, 1),
(3, 'Technical Standards', '/standards/', '_self', 2, 1),
(3, 'Reports & Research', '/publications/', '_self', 3, 1),
